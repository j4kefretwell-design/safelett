...
<html lang="en">
  ...
  <body style="margin:0;padding:0;background-color:#F8F4EE;font-family:Georgia,'Times New Roman',Times,serif;color:#33181C;">
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color:#F8F4EE;padding:32px 16px;">
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background-color:#F8F4EE;border:1px solid #C5B49D;">
            <tr>
              <td style="background-color:#33181C;padding:28px 32px;text-align:center;border-bottom:2px solid #C4A35A;">
                <p style="margin:0;font-family:Georgia,'Times New Roman',Times,serif;font-size:17px;font-weight:400;color:#F8F4EE;letter-spacing:0.22em;text-transform:uppercase;">
                  FRETWELL <span style="font-style:italic;color:#C4A35A;">&amp;</span> CO
                </p>
              </td>
            </tr>
            <tr>
              <td style="padding:36px 32px;background-color:#F8F4EE;">
                ${content}
              </td>
            </tr>
            <tr>
              <td style="padding:24px 32px;border-top:1px solid #C5B49D;background-color:#F8F4EE;text-align:center;">
      ...
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background-color:#F8F4EE;border:1px solid #C5B49D;">
            ...
            <tr>
              <td style="padding:36px 32px;background-color:#F8F4EE;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
                ${content}
              </td>
            </tr>
            <tr>
              <td style="padding:24px 32px;border-top:1px solid #C4A35A;background-color:#F8F4EE;text-align:center;">
...
function moduleAlertBanner(module: "compliance" | "tenancy"): string {
  ...
  const accentColor = module === "compliance" ? "#33181C" : "#1B2339";
...
function buildContractorSection(
  ...
  return `
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color:#E4DDCC;border:1px solid #C5B49D;border-radius:8px;margin-bottom:24px;">
      <tr>
...
export function buildExpiryAlertEmail({
  ...
  const html = emailLayout(`
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color:#E4DDCC;border:1px solid #C5B49D;border-radius:8px;margin-bottom:24px;">
      <tr>
        <td style="padding:16px 20px;border-bottom:1px solid #C5B49D;">
    ...
    <a href="${dashboardUrl}" style="display:inline-block;background-color:#33181C;color:#F8F4EE;text-decoration:none;font-size:14px;font-weight:600;padding:12px 20px;border-radius:8px;">
      View in ${BRAND_NAME}
    </a>
  `);
...
export function buildTenancyAlertEmail({
  ...
  const html = emailLayout(`
    ...
    <a href="${dashboardUrl}" style="display:inline-block;background-color:#1B2339;color:#F8F4EE;text-decoration:none;font-size:14px;font-weight:600;padding:12px 20px;border-radius:8px;">
      View in ${BRAND_NAME}
    </a>
  `);
...
export function buildWelcomeEmail({ dashboardUrl }: WelcomeEmailParams) {
  ...
  const html = brandWelcomeEmailLayout(`
    ...
    <a href="${dashboardUrl}" style="display:inline-block;background-color:#33181C;color:#F8F4EE;text-decoration:none;font-size:13px;font-weight:400;letter-spacing:0.1em;text-transform:uppercase;padding:14px 24px;">
...
export function buildPasswordResetEmail({ resetUrl }: { resetUrl: string }) {
  ...
  const html = emailLayout(`
    ...
    <a href="${safeResetUrl}" style="display:inline-block;background-color:#33181C;color:#F8F4EE;text-decoration:none;font-size:13px;font-weight:500;letter-spacing:0.08em;text-transform:uppercase;padding:14px 24px;border-radius:4px;">
      Reset password
    </a>
